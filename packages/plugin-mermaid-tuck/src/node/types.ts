 ...
 * {@link MermaidThemeVariables} type.
 *
 ...
 *
 * Extends `Record<string, unknown>` so it can be merged with other plugin
 * locale data by the toolkit's locale utilities.
 *
 * 继承 `Record<string, unknown>`，以便通过 toolkit 的多语言工具与其他插件的
 * 语言数据合并。
 */
export interface MermaidLocaleData extends Record<string, unknown> {
...
/**
 * Options accepted by the mermaid plugin factory.
 *
 * mermaid 插件工厂函数所接受的选项。
 *
 * @property options - Mermaid runtime configuration / Mermaid 运行时配置
 * @property locales - Per-locale UI strings keyed by locale code / 按语言代码索引的界面文案
 */
